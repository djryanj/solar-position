...
const userValidationRules = () => {
    return [
        ...
        check('interval')
        .trim().stripLow()
        .matches(/(^\d*)([hms]?$)/gi).withMessage('interval must be in a format like nnnI, where nnn is the number and I is one of s (seconds), m (minutes), or h (hours).')
        .custom((value, {
            req
        }) => {
            const newReq = req.method === "GET" ? req.query : req.body;
            const lastChar = value[value.length-1].toLowerCase();
            const intervalNumber = parseInt(value.substring(0,value.length-1));
